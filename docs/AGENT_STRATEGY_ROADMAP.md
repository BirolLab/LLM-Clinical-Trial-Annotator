...
| Field | Target | Full-corpus result | 95% CI | vs Human IRA | Status |
|---|---|---|---|---|---|
| classification | ≥95% | **96.8%** (513/530) | ±1.5pp | +5.2pp | ✅ SHIP |
| peptide | ≥85% | **86.8%** (466/537) | ±2.9pp | +38.4pp | ✅ SHIP |
| delivery_mode | ≥80% | **87.7%** (448/511) | ±2.9pp | +19.5pp | ✅ SHIP |
| outcome | ≥65% | **42.3%** (143/338) | ±5.3pp | **−13.3pp** | ❌ ACCEPT-with-bounded-claim (recency-mechanic, see §1.1) |
| reason_for_failure | ≥95% | 86.2% recall / 94.9% precision-when-emitted | ±12.6pp recall | −5.1pp recall / +3.6pp precision | ⚠️ SHIP-WITH-FLAG |
| sequence | ≥50% | 25.8% (94/364) | ±4.5pp | n/a | ❌ INVESTIGATE (lever 4 target) |

**Per-outcome-class breakdown (full corpus, n=239 GT-scoreable):**
- positive: 54/119 = 45.4% (bottleneck — pos→unk recency mechanic; lever 3 + 5 targets)
- unknown: 65/83 = 78.3%
- terminated: 18/20 = 90.0%
- **failed - completed trial: 0/11 = 0.0%** ⚠️ systematic miss (lever 2 target — v42.8.2 lands on dev)
- **withdrawn: 6/6 = 100.0% ⭐**

### 1.1 Recency stratification (the dominant outcome accuracy axis)

| Slice | NCT vintage | Outcome accuracy |
|---|---|---|
| Job #102 (batch 1) | mostly pre-2021 | 49.3% |
| Job #103 (batch 2) | all 2021+ | ~31% |
...
Original gate slice numbers preserved for traceability — see `docs/PRODUCTION_GATE_REPORT.md §7`. Outcome on the gate read 60.7%, which led to the ship-with-flag certification on 2026-05-02. The gate was retired as the canonical headline once full-corpus scoring landed (gate over-represented pre-2021 NCTs; outcome dropped 18.4pp on broader corpus).
...
- **Commit:** `096edcd3` on main (v42.7.22 merge — CGRP disambiguation). Full v42.7.5–v42.7.22 shipped:
  ...
  - v42.7.18: `_KNOWN_SEQUENCES` expansion — three named peptide drugs added by drug code → canonical sequence (one with a shared alias to an existing entry). Sources: Job #97's 8/10 sequence-N/A misses on peptide=True trials. Sequences-only — `_KNOWN_PEPTIDE_DRUGS` deliberately untouched per `feedback_frozen_drug_lists.md`.
  - v42.7.19: `delivery_mode` ambiguous-keyword relevance gate. Cross-job analysis of Jobs #92/#95/#96/#97 showed 6 distinct NCTs with spurious-oral pattern from FDA Drugs / OpenAlex / placebo-comparator citations. `_AMBIGUOUS_KEYWORDS` (tablet/capsule) matches now require the citation to mention an experimental intervention name. Non-ambiguous keywords unaffected.
  - v42.7.20: `_classify_publication` default flipped from `trial_specific` to `general`. Cross-job analysis (Jobs #95-#98) showed `positive → unknown` is the dominant outcome miss class (9-12 misses per slice). Spot inspection of Job #98 misses revealed 7-9 pubs per trial defaulting to [TRIAL-SPECIFIC] under v41b's "default to trial_specific" rule, leading the LLM to systematically discount them. v42.7.20 requires an explicit trial signal (NCT match, "phase X", "first-in-human", "clinical trial", etc.) for the trial_specific tag. _TRIAL_SIGNALS extended with combined-phase markers ("phase 1/2", "phase 1b") and "clinical trial"/"clinical study".
  - v42.7.21: `_KNOWN_SEQUENCES` expansion from Job #98 misses — two named peptide drugs (a long-acting C-peptide analog and an octreotate analog) added by drug code → canonical sequence, plus biological-name aliases. Sequences-only.
  - v42.7.22: CGRP / calcitonin disambiguation. A migraine trial's intervention "Calcitonin Gene-Related Peptide" (37aa alpha-CGRP) was shadowed by the shorter "calcitonin" key (32aa, different drug). Same v42.6.18 root cause — longest-first iteration was already in place; the longer key was just missing. Added "calcitonin gene-related peptide" + "cgrp" aliases → the correct 37aa sequence.
...
2. **Aggregator defaults determine outcome more than rules do.** R6 "default → Other" in classification ate self-assembling-peptide AMPs. R8 "default → Unknown" in outcome ate tons. Before merging a new aggregator, **list every label the default catches and justify it on realistic evidence**, not just the happy path.
...
- Multi-drug experimental arms where each drug uses a different route (e.g. an intradermal vaccine + an oral capsule co-administered) → agent reports both routes; GT picks one. This is GT-quality / definition limitation, not a fixable bug. Documented in CONTINUATION_PLAN backlog #7 (EXPLORED + REJECTED).
- OpenFDA multi-formulation aggregation (a single active ingredient marketed as both a subcutaneous and an oral product) theoretically possible but Job #100 cases were dominated by multi-drug-arm above — not the OpenFDA cause.
...
- v42.7.18: aliases for three named peptide drugs
- v42.7.21: a long-acting C-peptide analog + an octreotate analog + aliases
- v42.7.22: CGRP / calcitonin disambiguation via longest-first iteration on the longer key
**Known issues (remaining):**
- Drug-code → biological-name resolution gap (v42.8 candidate, architectural). UniProt and DRAMP queries via `peptide_identity` agent fail on pharma drug codes because those databases index biological protein names, not pharma drug codes. Same root cause as outcome's positive-recall gap — needs RxNorm/DrugBank resolver layer.
...
| v42.7.18 | 9d5ec33d | `_KNOWN_SEQUENCES` expansion (three named peptide drugs) | Job #98 NO REGRESSIONS (target NCTs absent from slice-D) |
...
| v42.7.21 | 59c0be20 | _KNOWN_SEQUENCES: long-acting C-peptide analog + octreotate analog + aliases | Job #99 — no slice-E trials matched these drugs (predicted) |
| v42.7.22 | 59c0be20 | CGRP / calcitonin disambiguation fix | Job #99 — no CGRP trials in slice-E; tested in unit tests |
...
5. **Reduce 5-NCT smoke ceremony.** v42.7.x cycle had 4–5 smokes; some
   ...
   regression test pinned to specific past bugs (a vaccine over-call
   case, a GLP-1 case, a DBAASP-hit case) — to replace exploratory smokes.
...
- Having a pregate match `"NS"` (normal saline) and a product brand name as peptide sequences — no minimum-length guard, no word-boundary check.
