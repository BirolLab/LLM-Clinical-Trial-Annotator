#!/usr/bin/env python3
"""Select held-out-F for the cycle after Job #99 (v42.7.23+).

Per per-cycle held-out separation: each slice is used at most once. A
retired post-#95, B post-#96, C post-#97, D post-#98, E post-#99 (when
that completes). Held-out-F is the next slice (single-use).

Random seed 9595 (A/B/C/D/E used 4242/5252/6262/7373/8484).

Pool concern: after held-out-F, ~38 residual GT-scoreable candidates
remain. Beyond that, expand test-batch coverage or accept smaller
slice sizes (15 NCTs) to push further.
"""
...
CSV_PATH = REPO_ROOT / "docs" / "human_ground_truth_train_df.csv"
JSON_BASE = REPO_ROOT / "results" / "json"
TEST_BATCH = REPO_ROOT / "scripts" / "fast_learning_batch_50.txt"
...
EXISTING_D = HERE / "holdout_outcome_slice_d_v42_7_18.json"
EXISTING_E = HERE / "holdout_outcome_slice_e_v42_7_19.json"
...
def main() -> int:
    ...
    slice_d = load_slice(EXISTING_D)
    slice_e = load_slice(EXISTING_E)
    print(
        f"Excluding: {len(prior)} prior + {len(test_batch)} test-batch + "
        f"{len(slice_83)} #83 + {len(slice_a)} A + {len(slice_b)} B + "
        f"{len(slice_c)} C + {len(slice_d)} D + {len(slice_e)} E",
        file=sys.stderr,
    )
    excluded = prior | test_batch | slice_83 | slice_a | slice_b | slice_c | slice_d | slice_e
    ...
    print("\nResidual candidate pool (held-out-F):", file=sys.stderr)
    total_candidates = 0
    ...
    random.seed(9595)
    # Same outcome mix as held-out-D: positive-heavy, no terminated/failed.
    # The terminated/failed pools were exhausted by held-out-A/B/C.
    targets = {
        "terminated": 0,
    ...
    print(f"\nSelected {len(picks)} held-out-F NCTs:", file=sys.stderr)
    for o, t in targets.items():
        pool = candidates_by_outcome[o]
        taken = min(t, len(pool))
        print(f"  {o}: {taken}", file=sys.stderr)
    print(
        "\nNote: same positive+unknown mix as held-out-D. The pool is "
        "shrinking with each retired slice — track residual_count and "
        "consider expanding test-batch coverage if pool drops below 60.",
        file=sys.stderr,
    )
