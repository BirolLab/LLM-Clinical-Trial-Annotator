#!/usr/bin/env python3
"""Select held-out-C for v42.7.17+ validation.

Held-out-B (`holdout_outcome_slice_b_v42_7_14.json`) was used as Job #96
(v42.7.16 baseline). It surfaced the v42.7.13 over-correction, which
v42.7.17 fixes. Per the per-cycle held-out separation, validating
v42.7.17 against held-out-B is overfitting — we now know its specific
failure NCTs. Held-out-C is the fresh slice for v42.7.17 and beyond.

Selection criteria (extends pick_holdout_b...py):
  1. Both annotators agree on Outcome.
  2. GT outcome is NOT 'active' (GT/registry divergence trap).
  3. Excluded:
     - All 47 NCTs in #83 baseline
     - All 30 NCTs in held-out-A
     - All 25 NCTs in held-out-B
     - Prior-jobs NCTs (#78–#82s)
     - Test-batch NCTs

Random seed 6262 (held-out-A used 4242, B used 5252).
"""
...
def main() -> int:
    ...
    print(
        ...
        f"+ {len(slice_b)} held-out-B",
        file=sys.stderr,
    )
    excluded = prior | test_batch | slice_83 | slice_a | slice_b
    ...
    print("\nResidual candidate pool (held-out-C):", file=sys.stderr)
    total_candidates = 0
    ...
    print(f"\nSelected {len(picks)} held-out-C NCTs:", file=sys.stderr)
