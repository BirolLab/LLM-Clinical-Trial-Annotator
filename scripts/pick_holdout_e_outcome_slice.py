#!/usr/bin/env python3
"""Select held-out-E for v42.7.19+ validation.

Per per-cycle held-out separation: each slice is used at most once for
the cycle that produced it. A retired post-#95, B post-#96, C post-#97,
D post-#98 (assumed once Job #98 completes). Held-out-E is the next slice
(single-use).

Random seed 8484 (A/B/C/D used 4242/5252/6262/7373).
"""
...
def main() -> int:
    ...
    print(
        ...
        f"{len(slice_c)} C + {len(slice_d)} D",
        file=sys.stderr,
    )
    excluded = prior | test_batch | slice_83 | slice_a | slice_b | slice_c | slice_d
    ...
    print("\nResidual candidate pool (held-out-E):", file=sys.stderr)
    total_candidates = 0
    ...
    print(f"\nSelected {len(picks)} held-out-E NCTs:", file=sys.stderr)
